...
| `PLAN`                | The agent's step-by-step plan to address the intent. The `content` is a structured array of step objects. |
...
| `FINAL_RESPONSE`      | The final AI response. The `content` contains the `ConversationMessage` and `uiMetadata` (e.g., sources, suggestions). |
...
5.  **Final Response (`LLMStreamSocket` & `ObservationSocket`)**: The final stream of `TOKEN` events from the synthesis call constitutes the agent's answer to the user. When this stream ends, a `FINAL_RESPONSE` observation is recorded, containing the complete, persisted `ConversationMessage` as well as a `uiMetadata` object with structured data like sources and suggestions for the UI to render.
